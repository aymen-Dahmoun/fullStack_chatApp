...
import { useNavigation } from "@react-navigation/native"; // ⬅️ for navigating to CallScreen
...
export default function ChatScreen({ route }) {
  ...
  const handleStartCall = () => {
    if (!socket || !messenger?.id) return;

    // Navigate immediately to CallScreen and send offer
    ...
    socket.emit("call user", {
      ...
      from: user.id,
      name: user.username,
      signalData: null, // will be replaced later by WebRTC SDP
    });
  };
  ...
  return (
    <View className="flex-1 bg-white pt-12 dark:bg-gray-950">
      ...
      >
        ...
        <View className="flex-row items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950">
          ...
          >
            <Icon source="phone" color="white" size={24} />
          </TouchableOpacity>
        </View>
